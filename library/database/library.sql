...
CREATE TABLE `tblbookrequests` (
  ...
  `IssuedBookId` int(11) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COLLATE=latin1_swedish_ci;
...
CREATE TABLE `tblorders` (
  ...
  `StatusNote` mediumtext DEFAULT NULL,
...
ALTER TABLE `tblbookrequests`
  ADD PRIMARY KEY (`id`),
  ADD KEY `idx_book_requests_student_status` (`StudentId`,`Status`),
  ADD KEY `idx_book_requests_book_status` (`BookId`,`Status`);
...
ALTER TABLE `tblcart`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `uniq_cart_student_book` (`StudentId`,`BookId`),
  ADD KEY `idx_cart_student` (`StudentId`);
...
ALTER TABLE `tblorders`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `uniq_order_number` (`OrderNumber`),
  ADD KEY `idx_orders_student` (`StudentId`);
