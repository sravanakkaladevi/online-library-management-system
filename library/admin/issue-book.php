...
$studentSql="SELECT Status FROM tblstudents WHERE StudentId=:studentid LIMIT 1";
...
{
 $_SESSION['error']="Student account is inactive";
header('location:manage-issued-books.php');
exit;
}
...
$activeIssueQuery->bindParam(':studentid',$studentid,PDO::PARAM_STR);
$activeIssueQuery->bindParam(':bookid',$bookid,PDO::PARAM_INT);
$activeIssueQuery->execute();
if($activeIssueQuery->fetch(PDO::FETCH_ASSOC))
{
 $_SESSION['error']="This book is already issued to the selected student";
header('location:manage-issued-books.php');
exit;
}

$book=fetchBookWithInventory($dbh, $bookid);
if($book && (int)$book['availableQty']>0){
